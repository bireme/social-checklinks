	...
	<script type="text/javascript" src="js/modernizr.js"></script>        
        ...
        function gotoPage(elemId) {
            ...
            if (goto && isNumber(goto)) {
                postToUrl('<%=response.encodeRedirectURL("list.jsp")%>', {group:(goto - 1),lang:'<%=lang%>',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>'});
            } else {
                postToUrl('<%=response.encodeRedirectURL("list.jsp")%>', {group:'<%=lastGroup%>',lang:'<%=lang%>',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>'});
            }
        }
        </script>        
    </head>
		...
		<div class="navbar navbar-inverse navbar-fixed-top">
				...
				<span class="icon-bar"></span>
                            ...
                            <a class="brand" href="javascript:postToUrl('<%=response.encodeRedirectURL("list.jsp")%>', {group:'0',lang:'<%=lang%>',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>'});"><%=messages.getString("bireme_social_checklinks")%></a>
                                ...
                                <ul class="nav pull-right">
                                    <li class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            <li <%if(lang.equals("en")) {%> class="disabled"<%}%>><a href="javascript:postToUrl('<%=response.encodeRedirectURL("list.jsp")%>', {group:'<%=group%>',lang:'en',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>'});">English</a></li>
                                            <li <%if(lang.equals("pt")) {%> class="disabled"<%}%>><a href="javascript:postToUrl('<%=response.encodeRedirectURL("list.jsp")%>', {group:'<%=group%>',lang:'pt',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>'});">Português</a></li>
                                            <li <%if(lang.equals("es")) {%> class="disabled"<%}%>><a href="javascript:postToUrl('<%=response.encodeRedirectURL("list.jsp")%>', {group:'<%=group%>',lang:'es',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>'});">Español</a></li>
                                            <!--li<%if(lang.equals("fr")) {%> class="disabled"<%}%>><a href="javascript:postToUrl('<%=response.encodeRedirectURL("list.jsp")%>', {group:'<%=group%>',lang:'fr',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>'});">Francés</a></li-->
                                        </ul>
                                    </li>
                                    <li class="dropdown">
                                        <a href="#" class="dropdown-toggle" data-toggle="dropdown"><i class="icon-user icon-white"></i> <%=user %> <b class="caret"></b></a>
                                        <ul class="dropdown-menu">
                                            <li><a href="javascript:postToUrl('<%=response.encodeRedirectURL("index.jsp")%>', {group:'<%=group%>',lang:'<%=lang%>',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>'});"><i class="icon-off"></i> <%=messages.getString("logout")%></a></li>
                                        </ul>
                                    </li>
                                </ul>
                            </div><!--/.nav-collapse -->
                        ...
                        <div class="span3">
                            <div class="breadcrumb" style="margin-bottom: 0px;">
                                <ul class="unstyled" style="color:#0088cc;font-size:12px;">
                                    ...
                                    <li style="line-height:15px">URL: <%=urlFilter==null?messages.getString("all"):urlFilter%></li>                           
                                                ...
                                                <div class="dropdown-menu" style="padding: 15px; padding-bottom: 0px;">
                                                    <ul style="list-style: none; padding: 0px; margin: 0px;">
                                                        <li style="margin-bottom: 8px;">
                                                            <a href="javascript:postToUrl('<%=response.encodeRedirectURL("list.jsp")%>', {lang:'<%=lang%>',group:'0',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>'});"><%=messages.getString("all")%></a>                                                          
                                                        </li>                                                            
                                                        <li style="margin-bottom: 8px;">
                                                            <% for (String db : databases) { %>
                                                                <a href="javascript:postToUrl('<%=response.encodeRedirectURL("list.jsp")%>', {lang:'<%=lang%>',group:'0',dbFilter:'<%=db%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>'});"><%=db%></a>
                                                            <% } %>
                                                        </li>        
                                                    </ul>
                                                </div>
                                    ...
                                    <div class="nav-collapse">
                                        <ul style="list-style: none; padding: 0px; margin: 0px;">
                                            <li class="dropdown">
                                                ...
                                                <div class="dropdown-menu" style="padding: 15px; padding-bottom: 0px;">
                                                    <ul style="list-style: none; padding: 0px; margin: 0px;">
                                                        <li style="margin-bottom: 8px;"><a href="javascript:postToUrl('<%=response.encodeRedirectURL("list.jsp")%>', {lang:'<%=lang%>',group:'0',dbFilter:'<%=dbFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>'});"><%=messages.getString("all")%></a></li>
                                                        <li>
                                                            <form action='<%=response.encodeRedirectURL("list.jsp?lang="+ lang + "&group=0&dbFilter=" + dbFilter + "&urlFilter=" + urlFilter + "&collCenterFilter=" + collCenterFilter + "&order=" + order)%>' method="post" >
                                        ...
                                        <div class="nav-collapse">
                                            <ul style="list-style: none; padding: 0px; margin: 0px;">
                                                <li class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu" style="padding: 15px; padding-bottom: 0px;">
                                                        <ul style="list-style: none; padding: 0px; margin: 0px;">
                                                            <li style="margin-bottom: 8px;"><a href="javascript:postToUrl('<%=response.encodeRedirectURL("list.jsp")%>', {group:'<%=group%>',lang:'<%=lang%>',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',order:'<%=order%>'});"><%=messages.getString("all")%></a></li>
                                                            <%
                                    ...
                                    <div class="nav-collapse">
                                        <ul style="list-style: none; padding: 0px; margin: 0px;">
                                            <li class="dropdown">
                                                ...
                                                <div class="dropdown-menu" style="padding: 15px; padding-bottom: 0px;">
                                                    <ul style="list-style: none; padding: 0px; margin: 0px;">                                                        
                                                        <li style="margin-bottom: 8px;"><a href="javascript:postToUrl('<%=response.encodeRedirectURL("list.jsp")%>', {group:'<%=group%>',lang:'<%=lang%>',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'ascending'});"><%=messages.getString("ascending_order")%></a></li>
                                                        <li style="margin-bottom: 8px;"><a href="javascript:postToUrl('<%=response.encodeRedirectURL("list.jsp")%>', {group:'<%=group%>',lang:'<%=lang%>',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'descending'});"><%=messages.getString("descending_order")%></a></li>
                                                    </ul>
                                                </div>
                                            </li>
                                        </ul>
