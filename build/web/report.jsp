	...
	<script type="text/javascript" src="js/modernizr.js"></script>        
        ...
        function gotoPage(elemId) {
            ...
            if (goto && isNumber(goto)) {
                postToUrl('<%=response.encodeRedirectURL("report.jsp")%>', {lang:'<%=lang%>',group:'<%=group%>',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>',r_dbFilter:'<%=r_dbFilter%>',r_idFilter:'<%=r_idFilter%>',r_urlFilter:'<%=r_urlFilter%>',r_collCenterFilter:'<%=r_collCenterFilter%>',r_group:(goto - 1),r_dateFilter:'<%=r_dateFilter%>',r_userFilter:'<%=r_userFilter%>'});
            } else {
        ...
        function confirmUndo(undoId) {
            if (confirm('<%=messages.getString("undo_confirm")%>')) {                
                postToUrl('<%=response.encodeRedirectURL("UndoFixReportServlet")%>', {id:undoId, lang:'<%=lang%>',group:'<%=group%>',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>',r_dbFilter:'<%=r_dbFilter%>',r_idFilter:'<%=r_idFilter%>',r_urlFilter:'<%=r_urlFilter%>',r_collCenterFilter:'<%=r_collCenterFilter%>',r_group:'<%=r_group%>',r_dateFilter:'<%=r_dateFilter%>',r_userFilter:'<%=r_userFilter%>'});
            } else {
                // Do nothing!
            }
        }
        function confirmUndo2(undoId, database, cc, fixedUrl, brokenUrl) {
            if (confirm('<%=messages.getString("undo_confirm")%>')) {                
                postToUrl('<%=response.encodeRedirectURL("UndoFixFeedbackServlet")%>', {id:undoId, database:database, cc:cc, fixedUrl:fixedUrl, brokenUrl:brokenUrl, lang:'<%=lang%>',group:'<%=group%>',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>',r_dbFilter:'<%=r_dbFilter%>',r_idFilter:'<%=r_idFilter%>',r_urlFilter:'<%=r_urlFilter%>',r_collCenterFilter:'<%=r_collCenterFilter%>',r_group:'<%=r_group%>',r_dateFilter:'<%=r_dateFilter%>',r_userFilter:'<%=r_userFilter%>'});
            } else {
                // Do nothing!
            }
        }
                            ...
                            <ul class="nav pull-right">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li <%if(lang.equals("en")) {%> class="disabled"<%}%>><a href="javascript:postToUrl('<%=response.encodeRedirectURL("report.jsp")%>', {group:'<%=group%>',lang:'en',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>',r_dbFilter:'<%=r_dbFilter%>',r_idFilter:'<%=r_idFilter%>',r_urlFilter:'<%=r_urlFilter%>',r_collCenterFilter:'<%=r_collCenterFilter%>',r_group:'<%=r_group%>',r_dateFilter:'<%=r_dateFilter%>',r_userFilter:'<%=r_userFilter%>'});">English</a></li>
                                        <li <%if(lang.equals("pt")) {%> class="disabled"<%}%>><a href="javascript:postToUrl('<%=response.encodeRedirectURL("report.jsp")%>', {group:'<%=group%>',lang:'pt',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>',r_dbFilter:'<%=r_dbFilter%>',r_idFilter:'<%=r_idFilter%>',r_urlFilter:'<%=r_urlFilter%>',r_collCenterFilter:'<%=r_collCenterFilter%>',r_group:'<%=r_group%>',r_dateFilter:'<%=r_dateFilter%>',r_userFilter:'<%=r_userFilter%>'});">Português</a></li>
                                        <li <%if(lang.equals("es")) {%> class="disabled"<%}%>><a href="javascript:postToUrl('<%=response.encodeRedirectURL("report.jsp")%>', {group:'<%=group%>',lang:'es',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>',r_dbFilter:'<%=r_dbFilter%>',r_idFilter:'<%=r_idFilter%>',r_urlFilter:'<%=r_urlFilter%>',r_collCenterFilter:'<%=r_collCenterFilter%>',r_group:'<%=r_group%>',r_dateFilter:'<%=r_dateFilter%>',r_userFilter:'<%=r_userFilter%>'});">Español</a></li>
                                        <!--li <%if(lang.equals("fr")) {%> class="disabled"<%}%>><a href="javascript:postToUrl('<%=response.encodeRedirectURL("report.jsp")%>', {group:'<%=group%>',lang:'fr',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>',r_dbFilter:'<%=r_dbFilter%>',r_idFilter:'<%=r_idFilter%>',r_urlFilter:'<%=r_urlFilter%>',r_collCenterFilter:'<%=r_collCenterFilter%>',r_group:'<%=r_group%>',r_dateFilter:'<%=r_dateFilter%>',r_userFilter:'<%=r_userFilter%>'});">Francés</a></li-->
                                    </ul>
                                </li>
                                <li class="dropdown">
                    ...
                    <div class="span3">
                        <div class="breadcrumb" style="margin-bottom: 0px;">
                            <ul class="unstyled" style="color:#0088cc;font-size:12px;">
                                ...
                                <li style="line-height:15px">URL: <%=r_urlFilter==null?messages.getString("all"):r_urlFilter%></li>                           
                                            ...
                                            <div class="dropdown-menu" style="padding: 15px; padding-bottom: 0px;">
                                                <ul style="list-style: none; padding: 0px; margin: 0px;">
                                                    <li style="margin-bottom: 8px;">
                                                        <a href="javascript:postToUrl('<%=response.encodeRedirectURL("report.jsp")%>', {lang:'<%=lang%>',group:'<%=group%>',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>',r_idFilter:'<%=r_idFilter%>',r_urlFilter:'<%=r_urlFilter%>',r_collCenterFilter:'<%=r_collCenterFilter%>',r_group:'0',r_dateFilter:'<%=r_dateFilter%>',r_userFilter:'<%=r_userFilter%>'});"><%=messages.getString("all")%></a>                                                          
                                                    </li>                                                            
                                                    <li style="margin-bottom: 8px;">
                                                        <% for (String db : databases) { %>
                                                            <a href="javascript:postToUrl('<%=response.encodeRedirectURL("report.jsp")%>', {lang:'<%=lang%>',group:'<%=group%>',dbFilter:'<%=db%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>',r_dbFilter:'<%=db%>',r_idFilter:'<%=r_idFilter%>',r_urlFilter:'<%=r_urlFilter%>',r_collCenterFilter:'<%=r_collCenterFilter%>',r_group:'0',r_dateFilter:'<%=r_dateFilter%>',r_userFilter:'<%=r_userFilter%>'});"><%=db%></a>
                                                        <% } %>
                                                    </li>        
                                                </ul>
                                            </div>
                                ...
                                <div class="nav-collapse">
                                    <ul style="list-style: none; padding: 0px; margin: 0px;">
                                        <li class="dropdown">
                                            ...
                                            <div class="dropdown-menu" style="padding: 15px; padding-bottom: 0px;">
                                                <ul style="list-style: none; padding: 0px; margin: 0px;">
                                                    <li style="margin-bottom: 8px;"><a href="javascript:postToUrl('<%=response.encodeRedirectURL("report.jsp")%>', {lang:'<%=lang%>',group:'<%=group%>',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',collCenterFilter:'<%=collCenterFilter%>',order:'<%=order%>',r_dbFilter:'<%=r_dbFilter%>',r_idFilter:'<%=r_idFilter%>',r_collCenterFilter:'<%=r_collCenterFilter%>',r_group:'0',r_dateFilter:'<%=r_dateFilter%>',r_userFilter:'<%=r_userFilter%>'});"><%=messages.getString("all")%></a></li>
                                                    <li>
                                                        <form action='<%=response.encodeRedirectURL("report.jsp?lang=" + lang + "&group=" + group + "&dbFilter=" + dbFilter + "&idFilter=" + idFilter + "&urFilter=" + urlFilter + "&collCenterFilter=" + collCenterFilter + "&order=" + order + "&r_dbFilter=" + r_dbFilter + "&r_idFilter=" + r_idFilter + "&r_collCenterFilter=" +  r_collCenterFilter + "&r_group=0" + "&r_dateFilter=" + r_dateFilter + "&r_userFilter=" + r_userFilter)%>' method="post" >
                                    ...
                                    <div class="nav-collapse">
                                        <ul style="list-style: none; padding: 0px; margin: 0px;">
                                            <li class="dropdown">
                                                ...
                                                <div class="dropdown-menu" style="padding: 15px; padding-bottom: 0px;">
                                                    <ul style="list-style: none; padding: 0px; margin: 0px;">
                                                        <li style="margin-bottom: 8px;"><a href="javascript:postToUrl('<%=response.encodeRedirectURL("report.jsp")%>', {lang:'<%=lang%>',group:'<%=group%>',dbFilter:'<%=dbFilter%>',idFilter:'<%=idFilter%>',urlFilter:'<%=urlFilter%>',order:'<%=order%>',r_dbFilter:'<%=r_dbFilter%>',r_idFilter:'<%=r_idFilter%>',r_urlFilter:'<%=r_urlFilter%>',r_group:'0',r_dateFilter:'<%=r_dateFilter%>',r_userFilter:'<%=r_userFilter%>'});"><%=messages.getString("all")%></a></li>
                                                        <li>
                                                            <form action='<%=response.encodeRedirectURL("report.jsp?lang=" + lang + "&group=" + group + "&dbFilter=" + dbFilter + "&idFilter=" + idFilter + "&urlFilter=" + urlFilter + "&collCenterFilter=" + collCenterFilter + "&order=" + order + "&r_dbFilter=" + r_dbFilter + "&r_idFilter=" + r_idFilter + "&r_urlFilter=" + r_urlFilter + "&r_group=0" + "&r_dateFilter=" + r_dateFilter + "&r_userFilter=" + r_userFilter)%>' method="post" >
